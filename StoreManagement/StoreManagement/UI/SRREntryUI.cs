...
namespace StoreManagement.UI
{
    public partial class SRREntryUI : Form
    {
        ...
        //s------------------------------------------------------------------------
            ...
            private Dictionary<string, Dictionary<string, ItemIssueFromGRR>> itemIssuefromGRR = null;
        #endregion
        ...
        public SRREntryUI(string srrID):this()
        {
            IsEdit = true;
            srrToEdit = srrID;
        }

        public SRREntryUI(bool UserOrDeptAuthority,string srrID)
            : this()
        {
            IsEdit = true;
            srrToEdit = srrID;
        ...
        private void FillData()
        {
            ...
            {
                int n;
                requisitionDT = requisitionManager.GetSRRList("3", srrToEdit,null);
                if (srr == null)
                {
                    srr = new SRR();
                }
                ...
                if (requisitionDT.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in requisitionDT.Rows)
                    {
                        ...
                        storeReqDataGridView.Rows[n].Cells[11].Value = dr["SlNo"].ToString();

                        //fill the invoice items in dictionary
                        Product srrItem = new Product();
                        srrItem.ReqQty = Convert.ToDecimal(dr["Required"].ToString());
                        srrItem.IssueQty = Convert.ToDecimal(dr["Issued"].ToString());
                        srrItem.Remarks = dr["Remarks"].ToString();
                        srrItem.PCode = dr["Code"].ToString();
                        srrItem.PID = dr["SlNo"].ToString();
                        srr.Items.Add(dr["SlNo"].ToString().Trim(), srrItem);                        
                    }                    
                }
            }
            catch 
        ...
        private bool IsValid()
        {
            try
            {
                ...
                else if (string.IsNullOrEmpty(purposeTextBox.Text.Trim()))
                {
            ...
            foreach (DataGridViewRow gridItem in storeReqDataGridView.Rows)
            {
                if (gridItem.Cells[1].Value != null)
                {
                    ...
                    if (IsEdit && (gridItem.Cells[11].Value != null && !string.IsNullOrEmpty(gridItem.Cells[11].Value.ToString())))
                    {
                        ...
                        //Start:Issued Grr Items

                        if (itemIssuefromGRR != null && itemIssuefromGRR.ContainsKey(srrProduct.PCode.ToString()))
                        {
                            srrProduct.IssuedGRR.Add(srrProduct.PCode, itemIssuefromGRR[srrProduct.PCode]);
                        }
                        //End:Issued Grr Items
                ...
                if (storeReqDataGridView.Rows.Count > 0)
                {
                    foreach (DataGridViewRow dgRow in storeReqDataGridView.Rows)
                    {
                        switch (choice)
                        {
                            ...
                            case "6":
                                if (string.IsNullOrEmpty(dgRow.Cells[6].Value.ToString().Trim()) || Convert.ToDecimal(dgRow.Cells[6].Value.ToString().Trim()) <= 0)
                                {
                                    return true;
                                }
                                break;
        ...
        {
            if (storeReqDataGridView.SelectedRows.Count > 0)
            {
                if (IsEdit)
                {
                    ShowItemGRRStock(storeReqDataGridView.SelectedRows[0].Cells[9].Value.ToString(), storeReqDataGridView.SelectedRows[0].Cells[11].Value.ToString());
                }
                else
                {
                    ShowItemGRRStock(storeReqDataGridView.SelectedRows[0].Cells[9].Value.ToString(),null);
                }
            }
        }

        private void ShowItemGRRStock(string condition1,string condition2)
        {
            DataTable itemGRRnDT;
            itemGRRGridView.Rows.Clear();
            try
            {
                
                if (!IsEdit)
                {
                    itemGRRnDT = requisitionManager.GetItemGRRList("1", condition1,null);
                }
                else
                {
                    itemGRRnDT = requisitionManager.GetItemGRRList("2", condition1, condition2);
                }

                
                if (itemGRRnDT.Rows.Count > 0)
                {
                    int n;
                    foreach (DataRow dr in itemGRRnDT.Rows)
                    {
                        n = itemGRRGridView.Rows.Add();
                        itemGRRGridView.Rows[n].Cells[0].Value = dr["GRRID"].ToString();
                        itemGRRGridView.Rows[n].Cells[1].Value = dr["Item"].ToString();
                        itemGRRGridView.Rows[n].Cells[2].Value = dr["GRRStock"].ToString();
                        itemGRRGridView.Rows[n].Cells[4].Value = dr["GoodsID"].ToString();
                        itemGRRGridView.Rows[n].Cells[5].Value = dr["GrrDetaiID"].ToString();

                        if (IsEdit)
                        {
                            itemGRRGridView.Rows[n].Cells[3].Value = dr["IssuedQty"].ToString();
                            itemGRRGridView.Rows[n].Cells[6].Value = dr["SRRID"].ToString();
                            itemGRRGridView.Rows[n].Cells[7].Value = dr["SrrDetailID"].ToString();
                            itemGRRGridView.Rows[n].Cells[8].Value = dr["SrrFromGrrID"].ToString();
                        }
                    }
                }
            }
        ...
        private void itemGRRGridView_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            TextBox txtEdit = (TextBox)e.Control;
            if (itemGRRGridView.CurrentRow != null)
            {
                if (itemGRRGridView.CurrentCell.ColumnIndex == 2)
                {
                    txtEdit.KeyPress += IsNumber_Keypress;
                }                
            }
        ...
        private void issueButton_Click(object sender, EventArgs e)
        {
            if (storeReqDataGridView.SelectedRows.Count > 0)
            {
                storeReqDataGridView.SelectedRows[0].Cells[6].Value = GetGRRQty();
            }
        }

        private string GetGRRQty()
        {
            decimal qty = 0; 
            try
            {
                if (itemGRRGridView.Rows.Count > 0)
                {
                    if (itemIssuefromGRR == null)
                    {
                        itemIssuefromGRR = new Dictionary<string, Dictionary<string, ItemIssueFromGRR>>();
                    }
                    Dictionary<string, ItemIssueFromGRR> grrItem = new Dictionary<string, ItemIssueFromGRR>();

                    foreach (DataGridViewRow dgRow in itemGRRGridView.Rows)
                    {
                        if (dgRow.Cells[3].EditedFormattedValue != null && (!string.IsNullOrEmpty(dgRow.Cells[3].EditedFormattedValue.ToString().Trim()) && Convert.ToDecimal(dgRow.Cells[3].EditedFormattedValue.ToString().Trim()) > 0))
                        {
                            qty += Convert.ToDecimal(dgRow.Cells[3].EditedFormattedValue.ToString().Trim());

                            ItemIssueFromGRR itemIssued = new ItemIssueFromGRR();

                            itemIssued.IssuedQty = Convert.ToDecimal(dgRow.Cells[3].EditedFormattedValue.ToString());
                            itemIssued.GoodsID = dgRow.Cells[4].Value.ToString();
                            itemIssued.GRRId = dgRow.Cells[0].Value.ToString();
                            itemIssued.GrrDetailID = dgRow.Cells[5].Value.ToString();

                            if (IsEdit && (dgRow.Cells[8].Value != null && !string.IsNullOrEmpty(dgRow.Cells[8].Value.ToString())))
                            {
                                //itemIssued.SRRId = dgRow.Cells[6].Value.ToString(); 
                                itemIssued.SrrDetailID = dgRow.Cells[7].Value.ToString();
                                itemIssued.SrrFromGrrID = dgRow.Cells[8].Value.ToString();
                                itemIssued.Condition = "2";                                
                            }

                            grrItem.Add(dgRow.Cells[5].Value.ToString(), itemIssued);
                        }
                        else if (IsEdit && dgRow.Cells[3].EditedFormattedValue != null && (string.IsNullOrEmpty(dgRow.Cells[3].EditedFormattedValue.ToString().Trim()) || Convert.ToDecimal(dgRow.Cells[3].EditedFormattedValue.ToString().Trim()) <= 0))
                        {  
                            if (dgRow.Cells[8].Value != null && !string.IsNullOrEmpty(dgRow.Cells[8].Value.ToString()))
                            {
                                ItemIssueFromGRR itemIssued = new ItemIssueFromGRR();
                                //itemIssued.SRRId = dgRow.Cells[6].Value.ToString(); 
                                itemIssued.SrrFromGrrID = dgRow.Cells[8].Value.ToString();
                                itemIssued.Condition = "4";
                                grrItem.Add(dgRow.Cells[5].Value.ToString(), itemIssued);
                            }                            
                        }
                    }

                    //Checek wheather the item already added or not, so delete first and add again
                    if(itemIssuefromGRR.ContainsKey(itemGRRGridView.Rows[0].Cells[4].Value.ToString()))
                    {
                        itemIssuefromGRR.Remove(itemGRRGridView.Rows[0].Cells[4].Value.ToString());
                    }

                    itemIssuefromGRR.Add(itemGRRGridView.Rows[0].Cells[4].Value.ToString(), grrItem);
                }
                return qty.ToString();
            }
            catch

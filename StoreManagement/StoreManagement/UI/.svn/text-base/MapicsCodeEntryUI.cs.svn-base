...
{
    public partial class MapicsCodeEntryUI : Form
    {
        #region Local variables

        private MasterSetupManager setupManager = null;
        private CostHead mapicCode = null;
        ...
        {
            setupManager = new MasterSetupManager();
        }

        public MapicsCodeEntryUI()
        {
            InitializeComponent();
        }
        ...
        private void saveButton_Click(object sender, EventArgs e)
        {
            if (IsValid())
            {
                if(setupManager.MapicsCodeManagement(mapicCode))
                {
                    ClearAllFields();
                }
            }
        ...
        {
            if(string.IsNullOrEmpty(mapicCodeTextBox.Text.Trim()))
            {
                MessageBox.Show("Enter MPAICS code.");
                mapicCodeTextBox.Focus();
                return false;
            }
        ...
        private void SetValues()
        {
            if (mapicCode == null)
            {
                mapicCode = new CostHead();
            }

            mapicCode.MapicCode = mapicCodeTextBox.Text.Trim();
            mapicCode.Remarks = remarksTextBox.Text.Trim();

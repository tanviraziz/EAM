...
{
    public partial class MIS_PurchaseOrderPrintUI : Form
    {
        #region Veriables
            ...
            private string OrderTermsChoice = null;
            private DataTable purchaseOrdDT;
            private Int16 orderItems = 0;
        #endregion

        public MIS_PurchaseOrderPrintUI()
        {
            InitializeComponent();
            SetObjects();
        }

        public MIS_PurchaseOrderPrintUI(string orderNo, string termsChoice)
            : this()
        {
            orderToPrint = orderNo;

            //OrderTermesChoice 1 for Purchaser Dept. Terms and 2 for MIS Dept. Terms
            OrderTermsChoice = termsChoice;
        }
        ...
        private void MIS_PurchaseOrderPrintUI_Load(object sender, EventArgs e)
        {
            ShowPrintData();
        ...
        private void ShowPrintData()
        {
            ...
            if (purchaseOrdDT.Rows.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(purchaseOrdDT.Rows[0]["ReqRaisedBy"].ToString()))
                {
        ...
        //Set the terms and condition when reprint
        ...
        //show the terms and condition for new print
        private void ShowTermsConditions()
        {
            DataTable dt = null;

            switch (OrderTermsChoice)
            {
                case "1":   // for Purchase dept. purchase order terms
                    dt = purchaseManager.GetTermsConditionsList("2", null);
                    break;
                case "2":   // for MIS dept. purchase order terms
        ...
        private void PrintReport()
        {
            ...
            {
                rpt = new MISOldPurOrderPrintReport();
            }
            else
            {
                rpt = new PurOrderPrintReport();
            }
